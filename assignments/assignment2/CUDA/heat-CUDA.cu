...
int main( int argc, char *argv[] ) {
    ...
    double *dev_u, *dev_uhelp;

    // Allocation on GPU for matrices u and uhelp
    cudaMalloc((void**)&dev_u, np*np*sizeof(double));
    cudaMalloc((void**)&dev_uhelp, np*np*sizeof(double));
    ...
    double *dev_diff;
    double *dev_residual;
    double *dev_residual2;
    
    // Allocation on GPU for residuals and diff array
    cudaMalloc((void**)&dev_diff, np*np*sizeof(double));
    cudaMalloc((void**)&dev_residual, np*np*sizeof(double));
    cudaMalloc((void**)&dev_residual2, sizeof(double));

    int numblocks = ((np-2)*(np-2)) / maxBlockDim  + ((np*np)%(maxBlockDim)!=0);
    
    iter = 0;

    while(1) {
        gpu_Residual<<<Grid,Block>>>(dev_u, dev_uhelp, np, dev_diff);
        cudaDeviceSynchronize();  // Wait for compute device to finish.
        gpu_Reduction<<<numblocks,maxBlockDim>>>(dev_diff, dev_residual, (np-2)*(np-2));
        cudaDeviceSynchronize();  // Wait for compute device to finish.
        gpu_Reduction<<<1,numblocks>>>(dev_residual, dev_residual2, numblocks); 
        cudaDeviceSynchronize();  // Wait for compute device to finish.
    ...
    cudaMemcpy(&residual,dev_residual2,sizeof(double),cudaMemcpyDeviceToHost);
        // solution good enough ?
        if (residual < 0.00005) break;
